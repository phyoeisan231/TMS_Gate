...
{
    public partial class CtlTruckOut1 : UserControl
    {
        ...
        private void btnDetail_Click(object sender, EventArgs e)
        {
            if (sfComboBoxCard.SelectedItem != null)
            {
                ...
                CtlTruckOutDetail ctl1 = new CtlTruckOutDetail() { Dock = DockStyle.Fill };
                CtlTruckOutWBQ ctl2 = new CtlTruckOutWBQ() { Dock = DockStyle.Fill };

                if (outboundCheck != null)
                {
                    // Populate CtlInDetail with data
                    ctl1.FillData(outboundCheck);
                    ctl.dinpanel1.Controls.Add(ctl1);

                    // Populate CtlTruckWBQ only if condition met
                    if (!string.IsNullOrEmpty(outboundCheck.OutWeightBridgeID))
                    {
                        ctl2.FillData(outboundCheck);
                        ctl.dinpanel2.Controls.Add(ctl2);
                    }
                }
                else
                {
                    MessageBoxAdv.Show(this, "OutboundCheck data is missing.", "Gate Out", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            ...
            {
                MessageBoxAdv.Show(this, "Please choose card No.", "Gate Out", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
